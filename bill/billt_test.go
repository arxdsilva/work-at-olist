...
func (s *S) TestCalculateBillOnMixedHoursWithSeconds(c *check.C) {
	start := time.Date(2018, time.September, 4, 21, 57, 13, 0, time.UTC)
	end := time.Date(2018, time.September, 4, 22, 7, 13, 0, time.UTC)
	toPay, err := Calculate(start, end)
	c.Assert(err, check.IsNil)
	c.Assert(toPay, check.Equals, 0.54)
}
...
func (s *S) TestUnbHoursBetweenStartAndEndWithInvalidStart(c *check.C) {
	start := time.Date(2018, time.September, 4, 23, 0, 0, 0, time.UTC)
	end := time.Date(2018, time.September, 5, 7, 0, 0, 0, time.UTC)
	c.Assert(unbHoursBetweenStartAndEnd(start, end), check.Equals, false)
}

func (s *S) TestUnbHoursBetweenStartAndEndWithInvalidEnd(c *check.C) {
	start := time.Date(2018, time.September, 4, 21, 0, 0, 0, time.UTC)
...
func (s *S) TestUnbillableTimeBetweenSE(c *check.C) {
	...
	c.Assert(unbillableTimeBetweenSE(start, end), check.Equals, float64(540))
}
